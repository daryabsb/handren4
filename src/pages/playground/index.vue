<template>
  <div class="h-full w-full bg-indigo-400 overflow-hidden absolute flex items-center">
  ...
  <div ref="allElementsRef" class="container mx-auto h-full py-16 px-8 relative">
    <div class="flex w-full rounded-lg h-full lg:overflow-hidden overflow-auto lg:flex-row flex-col shadow-2xl">
        ...
        <div class="p-8 shadow-md relative bg-white">
          <div class="flex items-center">
            ...
            <button class="bg-indigo-100 text-indigo-400 ml-auto w-8 h-8 flex items-center justify-center rounded">
              <svg stroke="currentColor" class="w-4 h-4" viewBox="0 0 24 24" stroke-width="2.2" fill="none"
          ...
          <div class="mt-6 flex">
            <button class="bg-indigo-500 text-white py-2 text-sm px-3 rounded focus:outline-none">
              New Contact
            </button>
            <button class="ml-4 text-gray-600 py-2 text-sm px-3 rounded focus:outline-none border border-gray-400">
              New Task
            </button>
            <div class="relative ml-auto flex-1 pl-8 sm:block hidden">
              ...
              <svg stroke="currentColor" class="w-4 h-4 absolute right-0 top-0 mt-3 mr-2 text-gray-500" stroke-width="2"
      ...
      <div class="lg:w-1/2 bg-indigo-600 text-white flex flex-col">
        <div class="p-8 bg-indigo-700 flex items-center">
          ...
          <button class="bg-indigo-600 text-white py-2 text-sm px-3 rounded focus:outline-none">
            New Task
          </button>
        </div>
        <div class="p-8 flex flex-1 items-start overflow-auto">
          <div class="flex-shrink-0 text-sm sticky top-0">
            <div class="flex items-center text-white mb-3">
              Open <span class="italic text-sm ml-1 text-indigo-300">(10)</span>
            </div>
            <div class="flex items-center text-indigo-300 mb-3">
              In Progress <span class="italic text-sm ml-1">(8)</span>
            </div>
            <div class="flex items-center text-indigo-300">
              Closed <span class="italic text-sm ml-1">(4)</span>
            </div>
          </div>
  ...
  <div class="fixed h-full left-12 top-0 items-center flex space-x-6">
    <div
      ...
      <button ref="themeButtonsRef" v-for="(color, idx) in coloredBtns" :key="idx"
        :class="[makeColor(color), ' w-5 h-5 rounded-full mb-2 outline-none focus:outline-none']" :theme-button="color"
        @click="e => handleThemeButtonClick(e, color)"></button>
...
export default {
  setup() {
    const allElementsRef = ref(null);
    const themeButtonsRef = ref(null);
    const coloredBtns = ref([
      "gray",
      "red",
      "orange",
      "green",
    ...
    const makeColor = (color) => "bg-" + color + "-500";

    onMounted(() => {
      allElements.value = allElementsRef.value.querySelectorAll('*:not([theme-button]):not([class*="gray"])');
      themeButtons.value = themeButtonsRef.value.map(button => button.getAttribute('theme-button'));
    });

    // const handleThemeButtonClick = (e) => {
    //   changeTheme(e.target.getAttribute("theme-button"));
    // };
    const handleThemeButtonClick = (e, theme) => {
      changeTheme(theme);
    };
